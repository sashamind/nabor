...
- **Typography** — monospace font presets + upload your own `.ttf` / `.otf` / `.woff2`, adjustable size, weight (100–900), line height, tracking. The grid follows the measured character cell, so changing typography never distorts the image proportions
...
- Шрифты: базовые моноширинные + загрузка своих (`.ttf`, `.otf`, `.woff2`), кегль, толщина, интерлиньяж, трекинг — сетка пересчитывается по фактическому знакоместу, пропорции картинки не искажаются
